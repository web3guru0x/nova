...
class net(torch.nn.Module):
    ...
    def forward(self,
        ...
        if isinstance(mol_x, torch.Tensor):
            mol_x = mol_x.to(self.device, non_blocking=True)
        if isinstance(mol_x_feat, torch.Tensor):
            mol_x_feat = mol_x_feat.to(self.device, non_blocking=True)
        if isinstance(bond_x, torch.Tensor):
            bond_x = bond_x.to(self.device, non_blocking=True)
        if isinstance(atom_edge_index, torch.Tensor):
            atom_edge_index = atom_edge_index.to(self.device, non_blocking=True)
        if isinstance(clique_x, torch.Tensor):
            clique_x = clique_x.to(self.device, non_blocking=True)
        if isinstance(clique_edge_index, torch.Tensor):
            clique_edge_index = clique_edge_index.to(self.device, non_blocking=True)
        if isinstance(atom2clique_index, torch.Tensor):
            atom2clique_index = atom2clique_index.to(self.device, non_blocking=True)
        if isinstance(residue_x, torch.Tensor):
            residue_x = residue_x.to(self.device, non_blocking=True)
        if isinstance(residue_evo_x, torch.Tensor):
            residue_evo_x = residue_evo_x.to(self.device, non_blocking=True)
        if isinstance(residue_edge_index, torch.Tensor):
            residue_edge_index = residue_edge_index.to(self.device, non_blocking=True)
        if isinstance(residue_edge_weight, torch.Tensor):
            residue_edge_weight = residue_edge_weight.to(self.device, non_blocking=True)
        if isinstance(mol_batch, torch.Tensor):
            mol_batch = mol_batch.to(self.device, non_blocking=True)
        if isinstance(prot_batch, torch.Tensor):
            prot_batch = prot_batch.to(self.device, non_blocking=True)
        if isinstance(clique_batch, torch.Tensor):
            clique_batch = clique_batch.to(self.device, non_blocking=True)
        
        # Enable autocast for mixed precision operations
        with torch.cuda.amp.autocast(enabled=torch.cuda.is_available() and self.device != "cpu"):
            # Init variables        
            reg_pred = None
            cls_pred = None
            mcls_pred = None
            ...
            # MOLECULE-PROTEIN Layers with GPU optimizations
            for idx in range(self.total_layer):
                ...
                # Pool Drug - try to keep operations on GPU
                ...
                # Residual connections - all on GPU
            ...
            # Pool based on attn scores - keep on GPU
            row, col = atom2clique_index
            ...
            # Final predictions - keep on GPU until the last possible moment
            if self.regression_head:
            ...
            # Prepare attention dictionary - keep data on GPU
            attention_dict = {
